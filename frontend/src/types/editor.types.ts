...
export type CanvasRepresentation = {
  version: string
  objects: object[]
  backgroundImage: fabric.Image & { src: string }
}
...
export interface UseImageEditor
  ...
  setGuildelinesState: (payload: boolean) => void
}
...
export interface UseShapes {
  ...
  addImage: (image: File, options?: fabric.IImageOptions) => void
...
export interface UseCanvasOperations {
  ...
  undo: () => void
  redo: () => void
  loadFromJson: (canvasState: CanvasRepresentation) => void
  canvasToJson: () => string
  canvasToBase64: (options?: fabric.IDataURLOptions) => string | null
  initHistory: () => void
