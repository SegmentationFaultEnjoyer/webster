...
export function useCanvasOperations(
  ...
  const canvasToFormData = async (
    ...
    const base64 = canvasToBase64(options)
  ...
  const canvasToJson = () => {
    return JSON.stringify(canvas.toJSON(NON_DEFAULT_PROPS))
  }

  const canvasToBase64 = (options?: fabric.IDataURLOptions) => {
    resetZoom()
    // scaling up image and all objects to initial sizes and to get max quality
    const restoreSize = preserveOriginalSize(canvas)
  ...
  const loadFromJson = (state: CanvasRepresentation) => {
